	...
	return (
		<>
			...
			<div className="flex gap-[20px]">
				<input ref={inputRef} id="inputLength" value={value} type="number" className="w-[280px] min-w-[100px] h-[40px] rounded-[10px] pl-[15px] text-white bg-block-black font-inter-regular focus:outline-input" onChange={e => setValue(e.target.value)} />
				<button className="min-w-[96px] h-[40px] rounded-[10px] text-white bg-block-black font-inter-regular tracking-wider hover:bg-hover-pop-up hover:text-black transition-colors"
					onClick={() => {
			...
			<button className="bg-block-black max-w-[410px] h-[40px] font-inter-regular tracking-wider rounded-[10px] text-white hover:bg-hover-pop-up hover:text-black transition-colors"
				onClick={() => editText()}
				onKeyDown={e => {
