...
import { FocusDispatchContext, TextContext, TextDispatchContext } from "../../../pages/Test/model/Context";
...
export const ListLang: FC<ListLangProps> = ({ massiv }) => {
    ...
    const handleClick = (language: string): void => {
      if (dispatchFocus) {
        ...
        setTimeout(() => {
          if (dispatchFocus) dispatchFocus ({type: 'SearchModalEdit', boolean: false})
          startAnimate(true)
        }, 70)
        setTimeout(() => {
          if (dispatchText) dispatchText ({type: 'LanguageEdit', string: language})
        }, 170)
      }
    }
                      ...
                      {textSettings?.language == language && (
                        <svg className="mt-[2px]" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                          ...
                          <g mask="url(#mask0_339_115)">
                              <path className="group-hover:fill-black" d="M7.9585 15L3.2085 10.25L4.396 9.06251L7.9585 12.625L15.6043 4.97917L16.7918 6.16667L7.9585 15Z" fill="#8F8F8F"/>
                          </g>
                        </svg>
                      )}
                      {language}
